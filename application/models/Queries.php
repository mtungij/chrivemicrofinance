...
public function get_allcutomer($comp_id){
	...
	public function remove_sponser($sp_id){
		return $this->db->delete('tbl_sponser',['sp_id'=>$sp_id]);
	}

	public function get_loan_data_modify($loan_id){
	$data = $this->db->query("SELECT l.loan_id,l.loan_aprove,l.how_loan,l.session,l.day,lc.loan_name,l.customer_id,l.blanch_id,l.category_id,l.rate,l.disburse_day,l.reason,l.empl_id,lc.loan_price,lc.loan_perday,l.loan_status,l.fee_status FROM tbl_loans l LEFT JOIN tbl_loan_category lc ON lc.category_id = l.category_id WHERE loan_id = '$loan_id'");
	return $data->row();
}


public function update_loan_edit($loan_id,$data){
return $this->db->where('loan_id',$loan_id)->update('tbl_loans',$data);	
}

public function update_loacalgovernment($attach_id,$data){
	return $this->db->where('attach_id',$attach_id)->update('tbl_attachment',$data);
}

 public function get_loan_customer_desc($customer_id){
       	$data = $this->db->query("SELECT * FROM tbl_loans WHERE customer_id = '$customer_id' ORDER BY loan_id DESC");
       	return $data->row();
       }

        public function insert_collateral($data){
  ...
  //pagination record
   ...
   public function record_count_dafault($comp_id) {
       $this->db->where(['tbl_outstand_loan.comp_id'=>$comp_id]);
       return $this->db->count_all("tbl_outstand_loan");
   }
   ...
   public function fetch_default_loan($limit, $start,$comp_id) {
       $this->db->limit($limit, $start);
       $this->db->select([
            'c.customer_id',
            'c.f_name',
       ...
       $this->db->order_by('customer_id', 'DESC');
       $this->db->join('tbl_blanch b','b.blanch_id = ot.blanch_id','left');
       $this->db->join('tbl_customer c','c.customer_id = ot.customer_id','left');
       $this->db->join('tbl_loans l','l.loan_id = ot.loan_id','left');
       $this->db->join('tbl_loan_category lc','lc.category_id = l.category_id','left');
       ...
       $query = $this->db->get("tbl_outstand_loan ot");
       if ($query->num_rows() > 0) {
           foreach ($query->result() as $row) {
               $data[] = $row;
           }
   ...
   public function fetch_default_loan_blanch($limit, $start,$blanch_id) {
       $this->db->limit($limit, $start);
       $this->db->select([
            'c.customer_id',
            'c.f_name',
       ...
       $this->db->join('tbl_outstand o','o.loan_id = l.loan_id','left');
       $this->db->join('tbl_employee e','e.empl_id = l.empl_id','left');
       $this->db->where(['ot.blanch_id'=>$blanch_id]);
       $this->db->where(['ot.out_status'=> 'open']);
       $query = $this->db->get("tbl_outstand_loan ot");
       if ($query->num_rows() > 0) {
           foreach ($query->result() as $row) {
...
public function get_loan_customer_apply($customer_id){
       	$data = $this->db->query("SELECT l.loan_id,l.loan_code,l.how_loan,l.session,l.day,l.loan_status,lc.category_id,lc.loan_name,lc.loan_price,lc.loan_perday,lc.formular,l.reason FROM tbl_loans l LEFT JOIN tbl_customer c ON c.customer_id = l.customer_id LEFT JOIN tbl_blanch b ON b.blanch_id = l.blanch_id LEFT JOIN tbl_sub_customer sc ON sc.customer_id = c.customer_id LEFT JOIN tbl_account_type at ON at.account_id = sc.account_id LEFT JOIN tbl_loan_category lc ON lc.category_id = l.category_id LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id WHERE l.customer_id = '$customer_id' ORDER BY l.loan_id DESC");
       	return $data->row();
       }
    ...
    public function search_CustomerID_Blanch($customer_code,$comp_id,$blanch_id){
      ...
      $data = $this->db->get('tbl_customer c');
       ...
       public function get_loanDisbarsed($loan_id){
       	  $data = $this->db->query("SELECT l.loan_id,l.code,l.blanch_id,l.group_id,l.comp_id,l.customer_id,l.loan_aprove,l.loan_code,l.day,l.session,c.comp_name,c.comp_phone,cs.phone_no,l.how_loan FROM tbl_loans l JOIN tbl_company c ON c.comp_id = l.comp_id JOIN tbl_customer cs ON cs.customer_id = l.customer_id WHERE l.loan_id = '$loan_id'");
       	     return $data->row();
       }
    ...
    //  }
     ...
     public function get_loan_active_customer($customer_id){
     	$data = $this->db->query("SELECT l.loan_id,l.loan_int,l.restration,l.customer_id,ot.loan_stat_date,ot.loan_end_date,l.loan_status,l.day FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id  WHERE l.customer_id = '$customer_id' ORDER BY l.loan_id DESC LIMIT 1");
     	return $data->row();
     }


     public function get_total_pay_description($loan_id){
     $data = $this->db->query("SELECT * FROM tbl_pay p LEFT JOIN tbl_loans l ON l.loan_id = p.loan_id LEFT JOIN tbl_account_transaction at ON at.trans_id = p.p_method WHERE p.loan_id = '$loan_id' ORDER BY p.pay_id DESC LIMIT 2");
     return $data->result();
     }
...
public function get_totalLoanout($customer_id){
	...
	public function get_cash_transaction($comp_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT * FROM tbl_prev_lecod pr LEFT JOIN tbl_customer c ON c.customer_id = pr.customer_id LEFT JOIN tbl_blanch b ON b.blanch_id = pr.blanch_id LEFT JOIN tbl_employee e ON e.empl_id = pr.empl_id WHERE pr.comp_id = '$comp_id' AND date(pr.lecod_day) = '$date' ORDER BY prev_id DESC");
		 return $data->result();
	}

	public function get_cash_transaction_sum($comp_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(pr.loan_aprov) AS total_aprove,SUM(pr.depost) AS total_deposit,SUM(pr.double_dep) AS total_double FROM tbl_prev_lecod pr LEFT JOIN tbl_customer c ON c.customer_id = pr.customer_id LEFT JOIN tbl_blanch b ON b.blanch_id = pr.blanch_id LEFT JOIN tbl_employee e ON e.empl_id = pr.empl_id WHERE pr.comp_id = '$comp_id' AND date(pr.lecod_day) = '$date' ORDER BY prev_id DESC");
		 return $data->row();
	}
    ...
    //manager position
      ...
      public function get_blanch_loanpend($comp_id){
       ...
       public function get_loan_customer($customer_id){
       	$data = $this->db->query("SELECT l.loan_id,l.loan_code,lc.loan_name,lc.interest_formular,l.loan_aprove,l.day,l.session,l.loan_status,l.loan_int,l.restration,ot.loan_stat_date,ot.loan_end_date FROM tbl_loans l LEFT JOIN tbl_customer c ON c.customer_id = l.customer_id LEFT JOIN tbl_blanch b ON b.blanch_id = l.blanch_id LEFT JOIN tbl_sub_customer sc ON sc.customer_id = c.customer_id LEFT JOIN tbl_account_type at ON at.account_id = sc.account_id LEFT JOIN tbl_loan_category lc ON lc.category_id = l.category_id LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id WHERE l.customer_id = '$customer_id'");
       	return $data->result();
       }
...
public function get_totalLoanDoneGroup($group_id){
    ...
    public function get_sum_expences($comp_id){
     ...
     public function get_sum_expences_prev($comp_id,$from,$to){
    	$date = date("Y-m-d");
    	$data = $this->db->query("SELECT SUM(req_amount) AS total_expences FROM tbl_request_exp WHERE comp_id = '$comp_id' AND req_date between '$from' and '$to'");
    	 return $data->row();
    }

     public function get_sum_expences_prev_blanch($blanch_id,$from,$to){
    	$date = date("Y-m-d");
    	$data = $this->db->query("SELECT SUM(req_amount) AS total_expences FROM tbl_request_exp WHERE blanch_id = '$blanch_id' AND req_date between '$from' and '$to'");
    	 return $data->row();
    }
 ...
 public function get_blanchReced($comp_id,$from,$to){
 	$data  = $this->db->query("SELECT * FROM tbl_depost d LEFT JOIN tbl_loans l ON l.loan_id = d.loan_id LEFT JOIN tbl_customer c ON c.customer_id = d.customer_id LEFT JOIN tbl_account_transaction at ON at.trans_id = d.depost_method LEFT JOIN tbl_employee e ON e.empl_id = d.empl_id WHERE d.comp_id = '$comp_id' AND d.depost_day between '$from' and '$to'");
 	return $data->result();
 }

 public function get_blanchReced_blanch($blanch_id,$from,$to){
 	$data  = $this->db->query("SELECT * FROM tbl_depost d LEFT JOIN tbl_loans l ON l.loan_id = d.loan_id LEFT JOIN tbl_customer c ON c.customer_id = d.customer_id LEFT JOIN tbl_account_transaction at ON at.trans_id = d.depost_method LEFT JOIN tbl_employee e ON e.empl_id = d.empl_id WHERE d.blanch_id = '$blanch_id' AND d.depost_day between '$from' and '$to'");
 	return $data->result();
 }

 public function get_blanch_total_receved($comp_id,$from,$to){
 	$total = $this->db->query("SELECT SUM(depost) AS Total_Depost,SUM(double_amont) AS total_double FROM tbl_depost WHERE  comp_id = '$comp_id' AND depost_day between  '$from' and '$to'");
 	  return $total->row();
 }

  public function get_blanch_total_receved_blanch($blanch_id,$from,$to){
 	$total = $this->db->query("SELECT SUM(depost) AS Total_Depost,SUM(double_amont) AS total_double FROM tbl_depost WHERE  blanch_id = '$blanch_id' AND depost_day between  '$from' and '$to'");
 	  return $total->row();
 }
...
//total customer
 ...
 public function get_loan_collection($comp_id){
 	$loan_data = $this->db->query("SELECT pn.penart_paid,SUM(d.depost) AS total_depost,c.f_name,c.m_name,c.l_name,b.blanch_name,l.loan_id,l.loan_int,l.restration,l.loan_status,ot.loan_end_date,e.username,l.day,ot.loan_stat_date,SUM(d.double_amont) AS total_double  FROM tbl_loans l 
	 ...
	 WHERE  l.comp_id = '$comp_id' GROUP BY l.loan_id LIMIT 0");
 ...
 public function get_totalPaid_loan($comp_id){
 	$loan_paid = $this->db->query("SELECT SUM(depost) AS total_loan_depost,SUM(double_amont) AS total_double FROM tbl_depost WHERE comp_id = '$comp_id'");
...
public function get_deducted_blanch($blanch_id){
 ...
 public function get_total_deducted_income($comp_id){
  ...
  public function get_total_deducted_income_blanch($blanch_id){
 	$today = date("Y-m-d");
 ...
 public function get_total_deducted_income_prev($comp_id,$from,$to){
 	$today = date("Y-m-d");
 	$data = $this->db->query("SELECT SUM(deducted_balance) AS total_deducted FROM tbl_deducted_fee WHERE comp_id = '$comp_id' AND deducted_date between '$from' and '$to'");
 	return $data->row();
 }

 public function get_total_deducted_income_prev_blanch($blanch_id,$from,$to){
 	$today = date("Y-m-d");
 	$data = $this->db->query("SELECT SUM(deducted_balance) AS total_deducted FROM tbl_deducted_fee WHERE blanch_id = '$blanch_id' AND deducted_date between '$from' and '$to'");
 	return $data->row();
 }
...
public function get_comp_miamala_dada($comp_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT * FROM tbl_miamala m LEFT JOIN tbl_blanch b ON b.blanch_id = m.blanch_id LEFT JOIN tbl_account_transaction at ON at.trans_id = m.provider LEFT JOIN tbl_employee e ON e.empl_id = m.empl_id WHERE m.comp_id = '$comp_id' AND m.date = '$date'");
	return $data->result();
}

public function get_comp_miamala_dada_blanch($blanch_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT * FROM tbl_miamala m LEFT JOIN tbl_blanch b ON b.blanch_id = m.blanch_id LEFT JOIN tbl_account_transaction at ON at.trans_id = m.provider LEFT JOIN tbl_employee e ON e.empl_id = m.empl_id WHERE m.blanch_id = '$blanch_id' AND m.date = '$date'");
	return $data->result();
}


public function get_comp_miamala_prev_data($comp_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT * FROM tbl_miamala m LEFT JOIN tbl_blanch b ON b.blanch_id = m.blanch_id LEFT JOIN tbl_account_transaction at ON at.trans_id = m.provider LEFT JOIN tbl_employee e ON e.empl_id = m.empl_id WHERE m.comp_id = '$comp_id' AND m.date between '$from' and '$to'");
	return $data->result();
}

public function get_comp_miamala_prev_data_blanch($blanch_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT * FROM tbl_miamala m LEFT JOIN tbl_blanch b ON b.blanch_id = m.blanch_id LEFT JOIN tbl_account_transaction at ON at.trans_id = m.provider LEFT JOIN tbl_employee e ON e.empl_id = m.empl_id WHERE m.blanch_id = '$blanch_id' AND m.date between '$from' and '$to'");
	return $data->result();
}

public function get_total_miamala($comp_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(amount) AS total_amount_saving,SUM(rec_amount) AS total_remain FROM tbl_miamala WHERE comp_id = '$comp_id' AND date = '$date'");
	return $data->row();
}

public function get_total_miamala_blanch($blanch_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(amount) AS total_amount_saving,SUM(rec_amount) AS total_remain FROM tbl_miamala WHERE blanch_id = '$blanch_id' AND date = '$date'");
	return $data->row();
}

public function get_total_miamala_prev_data($comp_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(amount) AS total_amount_saving,SUM(rec_amount) AS total_remain FROM tbl_miamala WHERE comp_id = '$comp_id' AND date between '$from' and '$to'");
	return $data->row();
}

public function get_total_miamala_prev_data_blanch($blanch_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(amount) AS total_amount_saving,SUM(rec_amount) AS total_remain FROM tbl_miamala WHERE blanch_id = '$blanch_id' AND date between '$from' and '$to'");
	return $data->row();
}
...
public function get_miamala_depost($id){
	$miamala = $this->db->query("SELECT * FROM tbl_miamala m LEFT JOIN tbl_account_transaction at ON at.trans_id = m.provider WHERE m.id = '$id'");
	return $miamala->row();
}

function fetch_loan_list_data_withdrawal($customer_id)
 {
  $this->db->where('customer_id', $customer_id);
  $this->db->order_by('loan_id', 'DESC');
  $query = $this->db->query("SELECT * FROM tbl_loans l  WHERE l.customer_id = '$customer_id' ORDER BY l.loan_id DESC LIMIT 1");
  //$output = '<option value="">Select Active Loan</option>';
  foreach($query->result() as $row)
  {
   $output .= '<option value="'.$row->loan_id.'">'.$row->loan_code. "-" ."(Tsh.".number_format($row->loan_aprove).")".'</option>';
...
public function get_total_loan_pending($blanch_id){
	$data = $this->db->query("SELECT pt.loan_id,pt.total_pend,pt.date,l.loan_int,l.day,c.f_name,c.m_name,c.l_name,c.phone_no,b.blanch_name,b.blanch_id,pt.blanch_id FROM tbl_pending_total pt LEFT JOIN tbl_loans l ON l.loan_id = pt.loan_id LEFT JOIN tbl_blanch b ON b.blanch_id = pt.blanch_id LEFT JOIN tbl_customer c ON c.customer_id = pt.customer_id  WHERE pt.blanch_id = '$blanch_id' AND total_pend IS NOT FALSE ");
...
public function get_eploye_deposit($blanch_id){
 ...
 public function get_comp_miamala_dada_prev($comp_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT * FROM tbl_miamala m LEFT JOIN tbl_blanch b ON b.blanch_id = m.blanch_id LEFT JOIN tbl_account_transaction at ON at.trans_id = m.provider LEFT JOIN tbl_employee e ON e.empl_id = m.empl_id WHERE m.comp_id = '$comp_id' AND m.date between '$from'and '$to'");
	return $data->result();
}

public function get_total_miamala_prev($comp_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(amount) AS total_amount_saving,SUM(rec_amount) AS total_remain FROM tbl_miamala WHERE comp_id = '$comp_id' AND date between '$from' and '$to'");
	return $data->row();
}

public function get_comp_miamala_dada_prev_blanch($blanch_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT * FROM tbl_miamala m LEFT JOIN tbl_blanch b ON b.blanch_id = m.blanch_id LEFT JOIN tbl_account_transaction at ON at.trans_id = m.provider LEFT JOIN tbl_employee e ON e.empl_id = m.empl_id WHERE m.blanch_id = '$blanch_id' AND m.date between '$from'and '$to'");
	return $data->result();
}

public function get_total_miamala_prev_blanch($blanch_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(amount) AS total_amount_saving,SUM(rec_amount) AS total_remain FROM tbl_miamala WHERE blanch_id = '$blanch_id' AND date between '$from' and '$to'");
	return $data->row();
}

 public function get_deposit_miamala($comp_id){
 	$date = date("Y-m-d");
 	$data = $this->db->query("SELECT * FROM tbl_miamala_deposit md LEFT JOIN tbl_loans l ON l.loan_id = md.loan_id LEFT JOIN tbl_employee e ON e.empl_id = md.empl_id LEFT JOIN tbl_blanch b ON b.blanch_id = md.blanch_id LEFT JOIN tbl_customer c ON c.customer_id = l.customer_id WHERE md.comp_id = '$comp_id' AND md.date_trans = '$date'");
 	return $data->result();
 }

 public function get_deposit_miamala_blanch_data($blanch_id){
 	$date = date("Y-m-d");
 	$data = $this->db->query("SELECT * FROM tbl_miamala_deposit md LEFT JOIN tbl_loans l ON l.loan_id = md.loan_id LEFT JOIN tbl_employee e ON e.empl_id = md.empl_id LEFT JOIN tbl_blanch b ON b.blanch_id = md.blanch_id LEFT JOIN tbl_customer c ON c.customer_id = l.customer_id WHERE md.blanch_id = '$blanch_id' AND md.date_trans = '$date'");
 	return $data->result();
 }

  public function get_deposit_miamala_total($comp_id){
 	$date = date("Y-m-d");
 	$data = $this->db->query("SELECT SUM(md.amount) AS total_dep FROM tbl_miamala_deposit md LEFT JOIN tbl_loans l ON l.loan_id = md.loan_id LEFT JOIN tbl_employee e ON e.empl_id = md.empl_id LEFT JOIN tbl_blanch b ON b.blanch_id = md.blanch_id LEFT JOIN tbl_customer c ON c.customer_id = l.customer_id WHERE md.comp_id = '$comp_id' AND md.date_trans = '$date'");
 	return $data->row();
 }

 public function get_deposit_miamala_total_blanch_data($blanch_id){
 	$date = date("Y-m-d");
 	$data = $this->db->query("SELECT SUM(md.amount) AS total_dep FROM tbl_miamala_deposit md LEFT JOIN tbl_loans l ON l.loan_id = md.loan_id LEFT JOIN tbl_employee e ON e.empl_id = md.empl_id LEFT JOIN tbl_blanch b ON b.blanch_id = md.blanch_id LEFT JOIN tbl_customer c ON c.customer_id = l.customer_id WHERE md.blanch_id = '$blanch_id' AND md.date_trans = '$date'");
 	return $data->row();
 }
  ...
  public function get_deposit_miamala_comp($comp_id,$from,$to){
 	$date = date("Y-m-d");
 	$data = $this->db->query("SELECT * FROM tbl_miamala_deposit md LEFT JOIN tbl_loans l ON l.loan_id = md.loan_id LEFT JOIN tbl_employee e ON e.empl_id = md.empl_id LEFT JOIN tbl_blanch b ON b.blanch_id = md.blanch_id LEFT JOIN tbl_customer c ON c.customer_id = l.customer_id WHERE md.comp_id = '$comp_id' AND md.date_trans between '$from' and '$to'");
 	return $data->result();
 }

 public function get_deposit_miamala_total_comp($comp_id,$from,$to){
 	$date = date("Y-m-d");
 	$data = $this->db->query("SELECT SUM(md.amount) AS total_dep FROM tbl_miamala_deposit md LEFT JOIN tbl_loans l ON l.loan_id = md.loan_id LEFT JOIN tbl_employee e ON e.empl_id = md.empl_id LEFT JOIN tbl_blanch b ON b.blanch_id = md.blanch_id LEFT JOIN tbl_customer c ON c.customer_id = l.customer_id WHERE md.comp_id = '$comp_id' AND md.date_trans between '$from' and '$to'");
 	return $data->row();
 }

  public function get_deposit_miamala_blanch($blanch_id,$from,$to){
 	$date = date("Y-m-d");
 	$data = $this->db->query("SELECT * FROM tbl_miamala_deposit md LEFT JOIN tbl_loans l ON l.loan_id = md.loan_id LEFT JOIN tbl_employee e ON e.empl_id = md.empl_id LEFT JOIN tbl_blanch b ON b.blanch_id = md.blanch_id LEFT JOIN tbl_customer c ON c.customer_id = l.customer_id WHERE md.blanch_id = '$blanch_id' AND md.date_trans between '$from' and '$to'");
 	return $data->result();
 }

 public function get_deposit_miamala_total_blanch($blanch_id,$from,$to){
 	$date = date("Y-m-d");
 	$data = $this->db->query("SELECT SUM(md.amount) AS total_dep FROM tbl_miamala_deposit md LEFT JOIN tbl_loans l ON l.loan_id = md.loan_id LEFT JOIN tbl_employee e ON e.empl_id = md.empl_id LEFT JOIN tbl_blanch b ON b.blanch_id = md.blanch_id LEFT JOIN tbl_customer c ON c.customer_id = l.customer_id WHERE md.blanch_id = '$blanch_id' AND md.date_trans between '$from' and '$to'");
 	return $data->row();
 }
...
public function get_saving_miamala($id){
...
public function check_empl_privillage($position_id,$empl_id,$comp_id){
 ...
 public function get_previous_loan_with($comp_id,$from,$to){
 	$data = $this->db->query("SELECT * FROM tbl_outstand ot LEFT JOIN tbl_loans l ON l.loan_id = ot.loan_id JOIN tbl_blanch b ON b.blanch_id = l.blanch_id JOIN tbl_customer c ON c.customer_id = l.customer_id JOIN tbl_loan_category lc ON lc.category_id = l.category_id JOIN tbl_account_transaction at ON at.trans_id = l.method WHERE ot.loan_stat_date between '$from' and '$to' AND l.comp_id = '$comp_id'");
 	return $data->result();
 }

 public function get_previous_loan_with_blanch_data($blanch_id,$from,$to){
  ...
  public function get_previous_loan_with_total($comp_id,$from,$to){
  ...
  public function get_previous_loan_with_total_blanch($blanch_id,$from,$to){
 	$data = $this->db->query("SELECT SUM(l.loan_aprove) AS total_loan_aprove,SUM(l.loan_int) AS total_loan_int FROM tbl_outstand ot LEFT JOIN tbl_loans l ON l.loan_id = ot.loan_id JOIN tbl_blanch b ON b.blanch_id = l.blanch_id JOIN tbl_customer c ON c.customer_id = l.customer_id JOIN tbl_loan_category lc ON lc.category_id = l.category_id JOIN tbl_account_transaction at ON at.trans_id = l.method WHERE ot.loan_stat_date between '$from' and '$to' AND l.blanch_id = '$blanch_id'");
 	return $data->row();
 }
...
public function get_loan_withdrawal_today_blanch_general($blanch_id){
 ...
 public function get_outstand_loan_blanch($blanch_id){
  ...
  public function get_outstand_loan_company($comp_id,$from,$to){
  	$date = date("Y-m-d");
 ...
 public function filter_loan_default($comp_id,$from,$to){
 	$data = $this->db->query("SELECT * FROM tbl_outstand_loan ol LEFT JOIN tbl_loans l ON l.loan_id = ol.loan_id LEFT JOIN tbl_customer c ON c.customer_id = ol.customer_id LEFT JOIN tbl_employee e ON e.empl_id = l.empl_id LEFT JOIN tbl_outstand ot ON ot.loan_id = ol.loan_id LEFT JOIN tbl_loan_category lc ON lc.category_id = l.category_id LEFT JOIN tbl_account_transaction at ON at.trans_id = l.method LEFT JOIN tbl_blanch b ON b.blanch_id = ol.blanch_id WHERE ol.comp_id = '$comp_id' AND ol.out_status = 'open' AND ol.outstand_date between '$from' and '$to'");
 	return $data->result();
 }

 public function filter_loan_default_blanch($blanch_id,$from,$to){
 ...
 public function get_total_outStand($blanch_id){
  ...
  public function get_total_outStand_blanch($comp_id,$from,$to){
 ...
 public function get_total_outStand_blanch_data($blanch_id,$from,$to){
 ...
 public function get_total_outStandcomp($comp_id){
  ...
  public function get_total_outStand_blanch_datas($blanch_id){
 	$data = $this->db->query("SELECT SUM(ol.remain_amount) AS total_remain FROM tbl_outstand_loan ol WHERE ol.blanch_id = '$blanch_id' AND ol.out_status = 'open'");
 	return $data->row();
 }
 	...
 	public function get_cash_transaction_blanch($blanch_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT * FROM tbl_prev_lecod pr LEFT JOIN tbl_customer c ON c.customer_id = pr.customer_id LEFT JOIN tbl_blanch b ON b.blanch_id = pr.blanch_id LEFT JOIN tbl_employee e ON e.empl_id = pr.empl_id WHERE pr.blanch_id = '$blanch_id' AND date(pr.lecod_day) = '$date' ORDER BY prev_id DESC");
		 return $data->result();
	}
	...
	public function get_blanchTransaction_blanch($from,$to,$blanch_id){
	 ...
	 public function get_loanlejectBlanch($blanch_id){
       ...
       public function get_miamala_data($id){
        ...
        public function get_miamala_datas($mia_id){
       	$data = $this->db->query("SELECT * FROM tbl_miamala WHERE id = '$mia_id'");
       	return $data->row();
       }
...
public function get_deduction_amount_fee($loan_id){
	$data = $this->db->query("SELECT SUM(withdrow) AS total_fee FROM tbl_pay WHERE loan_id = '$loan_id'");
	return $data->row();
}
...
public function get_in_deposit_blanch($blanch_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(depost) AS total_deposit_in,SUM(double_amont) AS total_deouble FROM tbl_depost WHERE blanch_id = '$blanch_id' AND dep_status = 'withdrawal' AND date(deposit_day) = '$date'");
	return $data->row();
}
...
public function get_company_account($comp_id){
	$data = $this->db->query("SELECT SUM(comp_balance) AS total_comp_balance FROM tbl_ac_company WHERE comp_id = '$comp_id'");
	return $data->row();
}


public function get_disbursed_loan_data($comp_id){
	$data = $this->db->query("SELECT SUM(loan_aprove) AS total_disbursed,SUM(loan_int) AS total_with FROM tbl_loans WHERE comp_id = '$comp_id' AND loan_status = 'withdrawal'");
	return $data->row();
}

public function get_disbursed_loan_data_blanch($blanch_id){
	$data = $this->db->query("SELECT SUM(loan_aprove) AS total_disbursed,SUM(loan_int) AS total_with FROM tbl_loans WHERE blanch_id = '$blanch_id' AND loan_status = 'withdrawal'");
	return $data->row();
}


 public function get_deposi_category_daily($comp_id){
 	$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(d.depost) AS total_daily FROM tbl_depost d LEFT JOIN tbl_loans l ON l.loan_id = d.loan_id WHERE d.comp_id = '$comp_id' AND l.day = '1' AND d.depost_day = '$date'");
		return $data->row();
	}

	public function get_deposi_category_daily_blanch($blanch_id){
 	$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(d.depost) AS total_daily FROM tbl_depost d LEFT JOIN tbl_loans l ON l.loan_id = d.loan_id WHERE d.blanch_id = '$blanch_id' AND l.day = '1' AND d.depost_day = '$date'");
		return $data->row();
	}

	 public function get_deposi_category_weekly($comp_id){
 	$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(d.depost) AS total_weekly FROM tbl_depost d LEFT JOIN tbl_loans l ON l.loan_id = d.loan_id WHERE d.comp_id = '$comp_id' AND l.day = '7' AND d.depost_day = '$date'");
		return $data->row();
	}

	 public function get_deposi_category_weekly_blanch($blanch_id){
 	$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(d.depost) AS total_weekly FROM tbl_depost d LEFT JOIN tbl_loans l ON l.loan_id = d.loan_id WHERE d.blanch_id = '$blanch_id' AND l.day = '7' AND d.depost_day = '$date'");
		return $data->row();
	}

	 public function get_deposi_category_monthly($comp_id){
 	$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(d.depost) AS total_monthly FROM tbl_depost d LEFT JOIN tbl_loans l ON l.loan_id = d.loan_id WHERE d.comp_id = '$comp_id' AND l.day = '30' AND d.depost_day = '$date'");
		return $data->row();
	}

	 public function get_deposi_category_monthly_blanch($blanch_id){
 	$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(d.depost) AS total_monthly FROM tbl_depost d LEFT JOIN tbl_loans l ON l.loan_id = d.loan_id WHERE d.blanch_id = '$blanch_id' AND l.day = '30' AND d.depost_day = '$date'");
		return $data->row();
	}


	public function get_daily_withdrawal($comp_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(l.loan_aprove) AS withdrawal_daily FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id WHERE l.comp_id = '$comp_id' AND l.loan_status = 'withdrawal' AND ot.loan_stat_date = '$date' AND l.day = '1'");
		return $data->row();
	}

	public function get_daily_withdrawal_blanch($blanch_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(l.loan_aprove) AS withdrawal_daily FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id WHERE l.blanch_id = '$blanch_id' AND l.loan_status = 'withdrawal' AND ot.loan_stat_date = '$date' AND l.day = '1'");
		return $data->row();
	}

	public function get_weekly_withdrawal($comp_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(l.loan_aprove) AS withdrawal_weekly FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id WHERE l.comp_id = '$comp_id' AND l.loan_status = 'withdrawal' AND ot.loan_stat_date = '$date' AND l.day = '7'");
		return $data->row();
	}

	public function get_weekly_withdrawal_blanch($blanch_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(l.loan_aprove) AS withdrawal_weekly FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id WHERE l.blanch_id = '$blanch_id' AND l.loan_status = 'withdrawal' AND ot.loan_stat_date = '$date' AND l.day = '7'");
		return $data->row();
	}



	public function get_monthly_withdrawal($comp_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(l.loan_aprove) AS withdrawal_monthly FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id WHERE l.comp_id = '$comp_id' AND l.loan_status = 'withdrawal' AND ot.loan_stat_date = '$date' AND l.day = '30'");
		return $data->row();
	}


	public function get_monthly_withdrawal_blanch($blanch_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(l.loan_aprove) AS withdrawal_monthly FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id WHERE l.blanch_id = '$blanch_id' AND l.loan_status = 'withdrawal' AND ot.loan_stat_date = '$date' AND l.day = '30'");
		return $data->row();
	}

	public function get_faini_data($comp_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(receve_amount) AS total_faini FROM tbl_receve WHERE comp_id = '$comp_id' AND receve_day = '$date'");
		return $data->row();
	}

	public function get_faini_data_blanch($blanch_id){
		$date = date("Y-m-d");
		$data = $this->db->query("SELECT SUM(receve_amount) AS total_faini FROM tbl_receve WHERE blanch_id = '$blanch_id' AND receve_day = '$date'");
		return $data->row();
	}


	public function fetch_formular($category_id)
 {
  $this->db->where('category_id', $category_id);
  $this->db->order_by('category_id', 'ASC');
  $query = $this->db->get('tbl_loan_category');
  // $output = '<option value="">Select loan category</option>';
  foreach($query->result() as $row)
  {
   $output .= '<option value="'.$row->formular.'">'.$row->formular.'  </option>';
  }
  return $output;
 }

   public function fetch_loan_duration($category_id)
 {
  $this->db->where('category_id', $category_id);
  $this->db->order_by('category_id', 'ASC');
  $query = $this->db->get('tbl_loan_category');
  ...
  foreach($query->result() as $row)
  {
   if ($row->duration == '30') {
    $data = 'Monthly';
   }elseif ($row->duration == '7') {
   $data = 'Weekly';
   }elseif ($row->duration == '1') {
   	$data = 'Daily';
   }
   $output .= '<option value="'.$row->duration.'">'.$data.' / '.$row->from_repayment.' - '.$row->to_repayment.'</option>';
  }
  return $output;
 }


 public function fetch_fee($category_id)
 {
  $this->db->where('category_id', $category_id);
  $this->db->order_by('category_id', 'ASC');
  $query = $this->db->get('tbl_loan_category');
  ...
  foreach($query->result() as $row)
  {
   $output .= '<option value="'.$row->deduct_fee.'">'.$row->deduct_fee.'  </option>';
  }
  return $output;
 }

public function get_total_blanch_balance_miamala($comp_id){
	$data = $this->db->query("SELECT SUM(m.amount) AS total_blanch_miamala,b.blanch_name FROM tbl_miamala m LEFT JOIN tbl_blanch b ON b.blanch_id = m.blanch_id WHERE m.comp_id = '$comp_id' GROUP BY m.blanch_id");
	return $data->result();
}

public function get_total_blanch_balance_miamala_blanch($blanch_id){
	$data = $this->db->query("SELECT SUM(m.amount) AS total_blanch_miamala,b.blanch_name FROM tbl_miamala m LEFT JOIN tbl_blanch b ON b.blanch_id = m.blanch_id WHERE m.blanch_id = '$blanch_id' GROUP BY m.blanch_id");
	return $data->result();
}


public function get_total_miamala_total($comp_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(amount) AS total_amount_saving,SUM(rec_amount) AS total_remain FROM tbl_miamala WHERE comp_id = '$comp_id'");
	return $data->row();
}

public function get_total_miamala_total_blanch($blanch_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(amount) AS total_amount_saving,SUM(rec_amount) AS total_remain FROM tbl_miamala WHERE blanch_id = '$blanch_id'");
	return $data->row();
}


public function get_miamala_depost_record_data($dep_id){
	$data = $this->db->query("SELECT * FROM tbl_miamala_deposit WHERE prev_id = '$dep_id'");
	return $data->row();
}


public function get_deposit_transaction($comp_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(d.depost) AS totalDep,SUM(d.double_amont) AS totalDouble,at.account_name FROM tbl_depost d LEFT JOIN tbl_account_transaction at ON at.trans_id = d.depost_method WHERE d.comp_id = '$comp_id' AND d.depost_day = '$date' GROUP BY d.depost_method");
	return $data->result();
}

public function get_deposit_transaction_blanch_data($blanch_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(d.depost) AS totalDep,SUM(d.double_amont) AS totalDouble,at.account_name FROM tbl_depost d LEFT JOIN tbl_account_transaction at ON at.trans_id = d.depost_method WHERE d.blanch_id = '$blanch_id' AND d.depost_day = '$date' GROUP BY d.depost_method");
	return $data->result();
}

public function get_deposit_transaction_comp($comp_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(d.depost) AS totalDep,SUM(d.double_amont) AS totalDouble,at.account_name FROM tbl_depost d LEFT JOIN tbl_account_transaction at ON at.trans_id = d.depost_method WHERE d.comp_id = '$comp_id' AND d.depost_day between '$from' and '$to' GROUP BY d.depost_method");
	return $data->result();
}

public function get_deposit_transaction_blanch($blanch_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(d.depost) AS totalDep,SUM(d.double_amont) AS totalDouble,at.account_name FROM tbl_depost d LEFT JOIN tbl_account_transaction at ON at.trans_id = d.depost_method WHERE d.blanch_id = '$blanch_id' AND d.depost_day between '$from' and '$to' GROUP BY d.depost_method");
	return $data->result();
}


public function get_loan_with_summary($comp_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(l.loan_aprove) AS total_with,at.account_name FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id LEFT JOIN tbl_account_transaction at ON at.trans_id = l.method WHERE l.comp_id = '$comp_id'  AND ot.loan_stat_date = '$date' GROUP BY l.method");
	return $data->result();
}

public function get_loan_with_summary_blanch_data($blanch_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(l.loan_aprove) AS total_with,at.account_name FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id LEFT JOIN tbl_account_transaction at ON at.trans_id = l.method WHERE l.blanch_id = '$blanch_id'  AND ot.loan_stat_date = '$date' GROUP BY l.method");
	return $data->result();
}

public function get_loan_with_summary_comp($comp_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(l.loan_aprove) AS total_with,at.account_name FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id LEFT JOIN tbl_account_transaction at ON at.trans_id = l.method WHERE l.comp_id = '$comp_id'  AND ot.loan_stat_date between '$from' and '$to' GROUP BY l.method");
	return $data->result();
}

public function get_loan_with_summary_blanch($blanch_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(l.loan_aprove) AS total_with,at.account_name FROM tbl_loans l LEFT JOIN tbl_outstand ot ON ot.loan_id = l.loan_id LEFT JOIN tbl_account_transaction at ON at.trans_id = l.method WHERE l.blanch_id = '$blanch_id'  AND ot.loan_stat_date between '$from' and '$to' GROUP BY l.method");
	return $data->result();
}


public function get_other_income($comp_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(r.receve_amount) AS total_income,i.inc_name FROM  tbl_receve r LEFT JOIN tbl_income i ON i.inc_id = r.inc_id WHERE r.comp_id = '$comp_id' AND r.receve_day = '$date' GROUP BY r.inc_id");
	return $data->result();
}

public function get_other_income_blanch_data($blanch_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(r.receve_amount) AS total_income,i.inc_name FROM  tbl_receve r LEFT JOIN tbl_income i ON i.inc_id = r.inc_id WHERE r.blanch_id = '$blanch_id' AND r.receve_day = '$date' GROUP BY r.inc_id");
	return $data->result();
}

public function get_other_income_prev($comp_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(r.receve_amount) AS total_income,i.inc_name FROM  tbl_receve r LEFT JOIN tbl_income i ON i.inc_id = r.inc_id WHERE r.comp_id = '$comp_id' AND r.receve_day between '$from'and '$to' GROUP BY r.inc_id");
	return $data->result();
}

public function get_other_income_prev_blanch($blanch_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(r.receve_amount) AS total_income,i.inc_name FROM  tbl_receve r LEFT JOIN tbl_income i ON i.inc_id = r.inc_id WHERE r.blanch_id = '$blanch_id' AND r.receve_day between '$from'and '$to' GROUP BY r.inc_id");
	return $data->result();
}

public function get_other_income_total($comp_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(r.receve_amount) AS total_income_total,i.inc_name FROM  tbl_receve r LEFT JOIN tbl_income i ON i.inc_id = r.inc_id WHERE r.comp_id = '$comp_id' AND r.receve_day = '$date'");
	return $data->row();
}

public function get_other_income_total_blanch_data($blanch_id){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(r.receve_amount) AS total_income_total,i.inc_name FROM  tbl_receve r LEFT JOIN tbl_income i ON i.inc_id = r.inc_id WHERE r.blanch_id = '$blanch_id' AND r.receve_day = '$date'");
	return $data->row();
}

public function get_other_income_total_prev($comp_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(r.receve_amount) AS total_income_total,i.inc_name FROM  tbl_receve r LEFT JOIN tbl_income i ON i.inc_id = r.inc_id WHERE r.comp_id = '$comp_id' AND r.receve_day between '$from' and '$to'");
	return $data->row();
}

public function get_other_income_total_prev_blanch($blanch_id,$from,$to){
	$date = date("Y-m-d");
	$data = $this->db->query("SELECT SUM(r.receve_amount) AS total_income_total,i.inc_name FROM  tbl_receve r LEFT JOIN tbl_income i ON i.inc_id = r.inc_id WHERE r.blanch_id = '$blanch_id' AND r.receve_day between '$from' and '$to'");
	return $data->row();
}


public function get_blanch_list($comp_id){
	$data = $this->db->query("SELECT SUM(ba.blanch_capital) AS total_balance,b.blanch_name,b.blanch_id FROM tbl_blanch_account ba LEFT JOIN tbl_account_transaction at ON at.trans_id = ba.receive_trans_id LEFT JOIN tbl_blanch b ON b.blanch_id = ba.blanch_id WHERE ba.comp_id = '$comp_id' GROUP BY ba.blanch_id");
	return $data->result();
}


public function get_blanch_list_account($blanch_id){
	$data = $this->db->query("SELECT ba.blanch_capital,b.blanch_name,b.blanch_id,at.account_name FROM tbl_blanch_account ba LEFT JOIN tbl_account_transaction at ON at.trans_id = ba.receive_trans_id LEFT JOIN tbl_blanch b ON b.blanch_id = ba.blanch_id WHERE ba.blanch_id = '$blanch_id'");
	return $data->result();
}


public function get_blanch_list_account_balance($blanch_id){
	$data = $this->db->query("SELECT SUM(ba.blanch_capital) AS blanch_balance,b.blanch_name,b.blanch_id,at.account_name FROM tbl_blanch_account ba LEFT JOIN tbl_account_transaction at ON at.trans_id = ba.receive_trans_id LEFT JOIN tbl_blanch b ON b.blanch_id = ba.blanch_id WHERE ba.blanch_id = '$blanch_id'");
	return $data->row();
}
