...
class Admin extends CI_Controller {
	...
	$comp_balance = $this->queries->get_company_account($comp_id);
	$disbursed = $this->queries->get_disbursed_loan_data($comp_id);
...
;
	...
	public function view_blanch_panel($blanch_id){
		$this->load->model('queries');
		$comp_id = $this->session->userdata('comp_id');
        $blanch_capital = $this->queries->get_blanch_list($comp_id);
        $blanch_data = $this->queries->get_blanch_data($blanch_id);

        $blanch_balance = $this->queries->get_blanch_list_account_balance($blanch_id);
	    $disbursed = $this->queries->get_disbursed_loan_data_blanch($blanch_id);

	    $account_capital = $this->queries->get_blanch_list_account($blanch_id);
         //          echo "<pre>";
	     // print_r($blanch_balance);
	     //         exit();


		$this->load->view('admin/blanch_panel',['blanch_data'=>$blanch_data,'blanch_id'=>$blanch_id,'blanch_capital'=>$blanch_capital,'disbursed'=>$disbursed,'account_capital'=>$account_capital,'blanch_balance'=>$blanch_balance]);
	}
...
$sqldata="UPDATE `tbl_ac_company` SET `comp_balance`= '$total_remain' WHERE  `trans_id`='$pay_method'";
 ...
 //   return true; 
	...
	public function loan_category(){
		...
		$interest_formular = $this->queries->get_interestFormular($comp_id);
			   //           echo "<pre>";
			   // print_r($loan_category);
			   //      exit();
		$this->load->view('admin/loan_category',['loan_category'=>$loan_category,'interest_formular'=>$interest_formular]);
	}
	...
	public function create_loanCategory(){
		...
		$this->form_validation->set_rules('duration','interest duration','required');
		$this->form_validation->set_rules('from_repayment','from repayment duration','required');
		$this->form_validation->set_rules('to_repayment','to repayment','required');
		$this->form_validation->set_rules('formular','formular','required');
		$this->form_validation->set_rules('deduct_fee','deduct fee','required');
		$this->form_validation->set_error_delimiters('<div class="text-danger">','</div>');
		if ($this->form_validation->run()) {
			 $data = $this->input->post();
			   // print_r($data);
   ...
   return true;
		...
		public function customer_sponser($customer_id){
				...
				$region = $this->queries->get_region();
				   ...
				   // echo "</pre>";
				   //             exit();
	           $this->load->view('admin/customer_sponser',['customer_profile'=>$customer_profile,'blanch'=>$blanch,'sponser'=>$sponser,'loan_customer'=>$loan_customer,'account'=>$account,'customer_id'=>$customer_id,'region'=>$region]);
}
...
public function delete_sponser($sp_id,$customer_id){
        ...
        return redirect('admin/customer_sponser/'.$customer_id);
   ...
   return true;  
			...
			public function customer_profile($customer_id){
				...
				$loan_customer = $this->queries->get_loan_customer($customer_id);
				   ...
				   // echo "</pre>";
				   //             exit();
	           $this->load->view('admin/customer_profile',['customer_profile'=>$customer_profile,'blanch'=>$blanch,'sponser'=>$sponser,'loan_customer'=>$loan_customer,'customer_id'=>$customer_id]);
}
...
public function apply_loan($customer_id){
	...
	$this->load->view('admin/apply_loan',['loan_pendings'=>$loan_pendings,'customer_profile'=>$customer_profile,'customer_id'=>$customer_id,'loan_category'=>$loan_category]);
}
...
$this->load->view('admin/search_customer',['customer'=>$customer,'sponser'=>$sponser,'sponsers_data'=>$sponsers_data,'region'=>$region]);
    ...
    public function all_loans_details($customer_id){
				...
				$account = $this->queries->get_accountTYpe();
				   ...
				   // echo "</pre>";
				   //             exit();
	      $this->load->view('admin/all_loans',['customer_profile'=>$customer_profile,'blanch'=>$blanch,'sponser'=>$sponser,'loan_customer'=>$loan_customer,'account'=>$account,'customer_id'=>$customer_id]);
	       }
 ...
 public function edit_loan($loan_id){
     ...
     $customer_profile = $this->queries->get_customer_profileData($customer_id);
	  ...
	  $local_oficer = $this->queries->get_loacagovment_data($loan_id);
	    ...
	    //          exit();
		$this->load->view('admin/edit_loan',['loan_data'=>$loan_data,'blanch'=>$blanch,'loan_customer'=>$loan_customer,'loan_category'=>$loan_category,'loan_pendings'=>$loan_pendings,'customer_id'=>$customer_id,'blanch_id'=>$blanch_id,'sponser'=>$sponser,'collateral'=>$collateral,'local_oficer'=>$local_oficer,'customer_profile'=>$customer_profile]);
	}
	...
	if ($this->form_validation->run()) {
        ...
        $cat = $this->queries->get_loancategoryData($category_id);
             ...
             if ($input < $output) {
                ...
                return redirect('admin/edit_loan/'.$loan_id);
                }elseif($input > $zaidi){
 ...
 public function delete_sponser_edit_loan($sp_id,$customer_id,$loan_id){
    ...
    public function create_collateral_edit_loan($loan_id){
        	...
        	if ($this->form_validation->run()) {
        		...
        		return redirect('admin/edit_loan/'.$customer_id.'/'.$loan_id);
        	}
        	$this->edit_loan();
        }
    ...
    public function create_loanapplication($customer_id){
      ...
      $this->form_validation->set_rules($validation);
       ...
       $cat = $this->queries->get_loancategoryData($category_id);
             ...
             if ($input < $output) {
                $this->session->set_flashdata('error','Loan Amount is Less');
                return redirect('admin/apply_loan/'.$customer_id.'/'.$blanch_id);
                }elseif($input > $zaidi){
                    $this->session->set_flashdata('error','Loan Amount is Greater');
                    return redirect('admin/apply_loan/'.$customer_id);
              }elseif($session > $to_repayment){
             $this->session->set_flashdata('error','laon Repayment is Greater');
                return redirect('admin/apply_loan/'.$customer_id);
              }elseif ($session < $from_repayment) {
    ...
    public function view_LoanCustomerData($customer_id,$comp_id){
        ...
        public function view_Dataloan($customer_id,$comp_id){
         ...
         $inc_history = $this->queries->get_loanIncomeHistory($customer_id);
    	    ...
    	    // echo "</pre>";
    	    // exit();
    	$this->load->view('admin/view_loan_customer',['customer'=>$customer,'sponser_detail'=>$sponser_detail,'loan_form'=>$loan_form,'collateral'=>$collateral,'local_oficer'=>$local_oficer,'inc_history'=>$inc_history,'customer_profile'=>$customer_profile]);
    }
  ...
  $query = $this->db->query($sqldata);
   ...
   $this->load->view('admin/report_data',['data'=>$data,'null'=>$null,'without'=>$without]);
	...
	public function filter_loan_withdrawal(){
        ...
        if ($blanch_id == 'all') {
       $data = $this->queries->get_previous_loan_with($comp_id,$from,$to);
		$sum_loan_withdrawal = $this->queries->get_previous_loan_with_total($comp_id,$from,$to); 	
        }else{
		$data = $this->queries->get_previous_loan_with_blanch_data($blanch_id,$from,$to);
		$sum_loan_withdrawal = $this->queries->get_previous_loan_with_total_blanch($blanch_id,$from,$to);
		}
       // echo "<pre>";
	// 	print_r($sum_loan_withdrawal);
	// 	   exit();
	   ...
	   $blanch_data = $this->queries->get_blanch_data($blanch_id);
		...
		$this->load->view('admin/loan_withdrawal_date',['data'=>$data,'blanch'=>$blanch,'from'=>$from,'to'=>$to,'blanch_data'=>$blanch_data,'sum_loan_withdrawal'=>$sum_loan_withdrawal,'blanch_id'=>$blanch_id,'blanch_id'=>$blanch_id]);
	}


	public function print_loan_withdrawalFilter($blanch_id,$from,$to){
	 $this->load->model('queries');
	 $comp_id = $this->session->userdata('comp_id');
	 $compdata = $this->queries->get_companyData($comp_id);
	 if ($blanch_id == 'all') {
	 ...
	 $mpdf = new \Mpdf\Mpdf(['mode' => 'utf-8','format' => 'A4-L','orientation' => 'L']);
     $html = $this->load->view('admin/loan_withdrawal_dateReport',['compdata'=>$compdata,'blanch_data'=>$blanch_data,'data'=>$data,'sum_loan_withdrawal'=>$sum_loan_withdrawal,'from'=>$from,'to'=>$to,'blanch_id'=>$blanch_id],true);
     $mpdf->SetFooter('Generated By Brainsoft Technology');
     $mpdf->WriteHTML($html);
     $mpdf->Output();
	}	
...
public function previous_transfor(){
 ...
 public function cash_transaction(){
 	...
 	$dep_data = $this->queries->get_deposit_transaction($comp_id);
 	$saving_deposit = $this->queries->get_comp_miamala_dada($comp_id);
    $total_saving = $this->queries->get_total_miamala($comp_id);


    //loan withdrawal
    $loan_with = $this->queries->get_loan_with_summary($comp_id);
    $loan_fee = $this->queries->get_total_deducted_income($comp_id);

    //otherincome
    $income_other = $this->queries->get_other_income($comp_id);
    $total_inc = $this->queries->get_other_income_total($comp_id);

    //expenses
    $sum_expenses = $this->queries->get_sum_expences($comp_id);
 	   //  echo "<pre>";
 	   // print_r($sum_expenses);
 	   //       exit();
 	$this->load->view('admin/cash_transaction',['cash'=>$cash,'sum_depost'=>$sum_depost,'sum_withdrawls'=>$sum_withdrawls,'blanch'=>$blanch,'dep_data'=>$dep_data,'saving_deposit'=>$saving_deposit,'total_saving'=>$total_saving,'loan_with'=>$loan_with,'loan_fee'=>$loan_fee,'income_other'=>$income_other,'total_inc'=>$total_inc,'sum_expenses'=>$sum_expenses]);
 }
 ...
 $this->load->view('admin/cash_transaction_blanch',['blanch'=>$blanch,'data_blanch'=>$data_blanch,'blanch_id'=>$blanch_id,'from'=>$from,'to'=>$to,'blanch_id'=>$blanch_id,'total_comp_data'=>$total_comp_data,'blanch_data'=>$blanch_data,'dep_data'=>$dep_data,'saving_deposit'=>$saving_deposit,'total_saving'=>$total_saving,'loan_with'=>$loan_with,'loan_fee'=>$loan_fee,'income_other'=>$income_other,'total_inc'=>$total_inc,'sum_expenses'=>$sum_expenses]);
 }
 ...
 public function delete_depost_data($pay_id){
  ...
  //           echo "<pre>";
    ...
    //miamala record
    $mia_rec = $this->queries->get_miamala_depost_record_data($dep_id);
    $mia_id = $mia_rec->mia_id;
    
    $muamala_balance = $this->queries->get_miamala_datas($mia_id);
    $old_amount = $muamala_balance->amount;
    $return_mia_acc = $old_amount + $depost;
...
//  	   exit();
    ...
    $this->update_loan_statatus_adjust_out($loan_id);
 	...
 	if ($descriptions->description == 'SYSTEM / DEFAULT LOAN RETURN') {
      ...
      if ($pay_status == 'M') {
      $this->update_blanch_amount_outstand($comp_id,$blanch_id,$new_out_balance,$trans_id);
      $this->return_miamala_data_amount($mia_id,$return_mia_acc);
      }else{
       $this->update_blanch_amount_outstand($comp_id,$blanch_id,$new_out_balance,$trans_id);	
     }
   ...
   $this->return_miamala_data_amount($mia_id,$return_mia_acc);
 	...
 	$this->insert_blanch_amount_deposit($blanch_id,$deposit_new,$trans_id);
 	 ...
 	 if ($pay_status == 'M') {
 	$this->update_prev_record_data_delete($pay_id,$remain_depost);
 	$this->delete_miamala_delete_deposit($dep_id);
 	$this->update_deposit_record_data_delete($pay_id);
 ...
 public function delete_miamala_delete_deposit($dep_id){
  return $this->db->delete('tbl_miamala_deposit',['prev_id'=>$dep_id]);
 }

  public function return_miamala_data_amount($mia_id,$return_mia_acc){
    $sqldata="UPDATE `tbl_miamala` SET `amount`= '$return_mia_acc' WHERE `id`= '$mia_id'";
    // print_r($sqldata);
    //    exit();
     $query = $this->db->query($sqldata);
     return true;	
 ...
 public function update_prev_record_data_delete($pay_id){
 return $this->db->delete('tbl_prev_lecod',['pay_id'=>$pay_id]);
 }
 ...
 public function update_deposit_record_data_delete($pay_id){
  return $this->db->delete('tbl_depost',['dep_id'=>$pay_id]);
 }
  ...
  return true;
   ...
   public function print_general_reportBlanch($blanch_id){
	...
	public function get_blanch_receved(){
		...
		if ($blanch_id == 'all') {
		$data_blanch = $this->queries->get_blanchReced($comp_id,$from,$to); 
		$total_receve_blanch = $this->queries->get_blanch_total_receved($comp_id,$from,$to);	
		}else{
		$data_blanch = $this->queries->get_blanchReced_blanch($blanch_id,$from,$to); 
		$total_receve_blanch = $this->queries->get_blanch_total_receved_blanch($blanch_id,$from,$to);
       }

		$blanch_data = $this->queries->get_blanch_data($blanch_id);
		 //        echo "<pre>";
		 // print_r($total_receve_blanch);
		 //         exit();
		$this->load->view('admin/blanch_receved',['data_blanch'=>$data_blanch,'blanch'=>$blanch,'total_receve_blanch'=>$total_receve_blanch,'blanch_data'=>$blanch_data,'from'=>$from,'to'=>$to,'blanch_id'=>$blanch_id]);
	}
...
public function saving_deposit(){
	...
	$blanch_miamala = $this->queries->get_total_blanch_balance_miamala($comp_id);
	$total_blanch_miamala = $this->queries->get_total_miamala_total($comp_id);
	//  echo "<pre>";
	// print_r($total_blanch_miamala);
	//        exit();
	$this->load->view('admin/saving_deposit',['saving_deposit'=>$saving_deposit,'total_saving'=>$total_saving,'blanch'=>$blanch,'acount_trans'=>$acount_trans,'blanch_miamala'=>$blanch_miamala,'total_blanch_miamala'=>$total_blanch_miamala]);
}

function fetch_employee_blanchdata()
{
$this->load->model('queries');
if($this->input->post('blanch_id'))
{
 ...
 $this->db->query("INSERT INTO tbl_blanch_account (`comp_id`,`blanch_id`,`receive_trans_id`,`blanch_capital`) VALUES ('$comp_id','$blanch_id','$account','$saving')"); 	
  ...
  public function filter_miamala(){
   ...
   $blanch_data = $this->queries->get_blanch_data($blanch_id);
	...
	$this->load->view('admin/filter_miamala',['blanch'=>$blanch,'saving_deposit'=>$saving_deposit,'total_saving'=>$total_saving,'from'=>$from,'to'=>$to,'blanch_id'=>$blanch_id,'blanch_data'=>$blanch_data,'blanch_miamala'=>$blanch_miamala,'total_blanch_miamala'=>$total_blanch_miamala,'acount_trans'=>$acount_trans]);
}
...
public function deposit_lecord(){
    	...
    	$comp_id = $this->session->userdata('comp_id');
    	$blanch = $this->queries->get_blanch($comp_id);
        $dep_receord = $this->queries->get_deposit_miamala($comp_id);
        $total_rec = $this->queries->get_deposit_miamala_total($comp_id);
         //           echo "<pre>";
         // print_r($total_rec);
         //          exit();

    	$this->load->view('admin/deposit_lecord',['dep_receord'=>$dep_receord,'total_rec'=>$total_rec,'blanch'=>$blanch]);
    }
    ...
    public function filter_deposit_miamala(){
    	...
    	$this->load->view('admin/filter_deposit_miamala',['dep_receord'=>$dep_receord,'total_rec'=>$total_rec,'from'=>$from,'to'=>$to,'blanch'=>$blanch,'blanch_id'=>$blanch_id,'blanch_data'=>$blanch_data]);
    }
    ...
    $this->load->view('admin/deposit_saving',['data_saving'=>$data_saving,'customer'=>$customer,'date_data'=>$date_data]);
    }
...
public function Default_loan(){
	...
	$data['total_default'] = $this->queries->get_total_outStand_comp($comp_id);
	
	$data['blanch'] = $this->queries->get_blanch($comp_id);
    $config = array();
       $config["base_url"] = base_url() . "admin/Default_loan";
    ...
    $config['num_tag_close'] = '</span></li>';
       ...
       $data["default"] = $this->queries->
           fetch_default_loan($config["per_page"], $page,$comp_id);
       $data["links"] = $this->pagination->create_links();


	// print_r($blanch);
	//     exit();
	$this->load->view('admin/default_loan',$data);
}
...
public function filter_default_loan(){
	...
	$this->load->view('admin/filter_defaultloan',['blanch'=>$blanch,'default_blanch'=>$default_blanch,'total_blanch'=>$total_blanch,'blanch_data'=>$blanch_data,'from'=>$from,'to'=>$to,'blanch_id'=>$blanch_id]);
}
