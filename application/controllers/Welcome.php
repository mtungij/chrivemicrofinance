...
// user sing in
    ...
    $this->load->helper('string');
      ...
      //  $admin_otp = 'A-'. random_string('numeric',4);
      //  $comp_id = $this->session->userdata('comp_id');	
      //  $company_data = $this->queries->get_companyData($comp_id);
      //  $comp_phone = $company_data->comp_phone;
      //  //$empl_name = $manager_data->empl_name;

      // $this->update_admin_verfication_code($comp_id,$admin_otp);
      //   $phone = $comp_phone; 
      //   $massage = 'One Time Password ' . $admin_otp;
     // print_r($massage);
     //         exit(); 
      ...
      }elseif ($userexit == FALSE) {
     //Employe loagin
      $employee = $this->queries->employee_user_data($empl_no,$password);
      if ($employee){
				if ($employee) {
					$sessionData = [
					...
					'position_id' => $employee->position_id,
					 ...
					 //	echo "<prblocked_menue>";
			     // print_r($sessionData);
			     //           exit();
			if ($employee->empl_status == 'open'){
       $this->session->set_userdata($sessionData);              	   
      //  $empl_otp = 'E-'. random_string('numeric',4);
        $empl_id = $this->session->userdata('empl_id');	
        $manager_data = $this->queries->get_manager_data($empl_id);
      //  $comp_id = $manager_data->comp_id;
      //  $empl_no = $manager_data->empl_no;
      //  $company_data = $this->queries->get_companyData($comp_id);
      //  $comp_phone = $company_data->comp_phone;
      //  $empl_name = $manager_data->empl_name;

      // $this->update_empl_verfication_code($empl_id,$empl_otp);
      //   $full_name = $empl_name;
      //   $phone = $empl_no; 
      //   $massage = 'One Time Password ' . $empl_otp .' ' .'For' . ' ' . $full_name;
     // print_r($massage);
     //         exit(); 
               //$this->sendsms($phone,$massage);
                    $this->session->set_flashdata('massage','login successfully');
                    $this->session->set_userdata($sessionData);
                       if ($manager_data->position_id == '22') {
                      return redirect('admin/index');
                       }else{
                      	return redirect('oficer/index');
                      	}
                      }elseif ($employee->empl_status == 'close') {
